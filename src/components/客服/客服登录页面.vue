...
export default {
    ...
    methods:
    {
        ...
        向后端发送登录请求() {
             ...
             * 
             * 后端返回值：
             * 用户id：customer_server_id
            */
            var that = this;//用来保存当前的对象
            if (this.input_account != '' && this.input_password != '') {
                this.axios
                    .get(`/customer_server/login?name=${this.input_account}&password=${this.input_password}`)//向后端接口传输
                    .then((Return_info) => {
                        if (Return_info.data == "invalid") {
                            this.Alert_Error("账号或密码错误！")
                        }
                        else {
                            this.customer_server_id = Return_info.data//通过后端返回值修改用户id
                            this.跳转到总界面()//跳转到商品页面
                        }
                    })
            }
        ...
        跳转到总界面() {
            this.$router.push({
                path: '/客服/Page',
                query: {
                    customer_server_id: this.customer_server_id
                }
            })
        },
        跳转到注册页面() {
        ...
        一键登录() {
            this.customer_server_id = "271dcbf40c9d464398507031343c0ead"//强行修改id（对应账号jw21811）
            this.跳转到总界面()
        },
        返回() {
            this.$router.push({
