    ...
    <div class="背景">
        <div class="单个订单区域" v-for="(Order, index) in 订单总表">
                <div class="划分区域_子类纵向排列">
                    ...
                    <div>
                        <el-button type="primary" @click="确认完成制作(Order.order.id)">完成制作</el-button>
                    </div>
                </div>
                <div class="划分区域_子类纵向排列">
                    <div>配送地址：<br>{{ Order.order.customer_address }}</div>
                    <div>订单总价：<br>{{ Order.order.total_price }} 元</div>
                    <div>订单备注：<br>{{ Order.order.notes }} </div>
                </div>
                <div class="划分区域_子类纵向排列">
                    <div>品名:</div>
...
export default {
    ...
    methods:    
    {
        ...
        向后端修改订单状态(id) {
            var address = '/merchant/setOrderPreparing?merchant_id=' + this.merchant_id + '&order_id=' + id
            console.log(address)
            this.axios
                .get(address)//向后端接口传输
                .then((Return_info) => {
